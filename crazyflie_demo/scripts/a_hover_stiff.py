    ...
    def hover(self, xr, yr, zr, goal_r):
        ...
        while t < P.t_end:
            ...
            # for _ in range(100):
                ...
                #     ze_hist += (ze * self.t_phys) # Find integral component
                # ze_der = (ze - ze_prev) / self.t_phys # Find derivative component
                # ze_prev = ze
                # ze_tot = self.z_feed_forward + (ze * self.z_kp) + (ze_hist * self.z_ki) \
                #     + (ze_der * self.z_kd) # Eq. 3.1.7 Sum PID errors and multiply by gains
                self.msg.linear.z = altitude_ctrl_phys.update(z, r[2,0], self.t_phys)
